...
class EventDataset(Dataset):
    def __init__(self, samples_folder,  # Folder from where to load the data
        ...
        if dataset_name == 'DVS128':
            ...
            self.labels = np.array(
                [int(t[5:7]) for s in self.samples for t in s.split('_') if 'label' in t]
            ).astype('int8')
            self.unique_labels = {l: i for i, l in enumerate(sorted(set(self.labels)))}
            self.labels = [self.unique_labels[l] for l in self.labels]
            self.num_classes = len(self.unique_labels)
    ...
    def __getitem__(self, idx, return_sparse_array=False):
        ...
        while sf_num >= 0:
            ...
            # Get patch-center pixel locations with a vectorized path.
            # This avoids a worker-side Python list construction that has been
            # unstable in the EHW training path under multiprocessing.
            row_centers = np.arange(
                self.patch_size // 2,
                chunk_candidate.shape[0],
                self.patch_size,
                dtype=np.int64,
            )
            col_centers = np.arange(
                self.patch_size // 2,
                chunk_candidate.shape[1],
                self.patch_size,
                dtype=np.int64,
            )
            grid_rows, grid_cols = np.meshgrid(row_centers, col_centers, indexing='ij')
            pixels = np.stack((grid_rows.reshape(-1), grid_cols.reshape(-1)), axis=1)
...
class Event_DataModule(LightningDataModule):
    ...
    def _require_split_folder(self, split):
        split_folder = self.data_folder + split + '/'
        if not os.path.isdir(split_folder):
            raise FileNotFoundError(f"Split folder not found: {split_folder}")
        split_files = [f for f in os.listdir(split_folder) if f.endswith('.pckl')]
        if len(split_files) == 0:
            raise FileNotFoundError(f"No .pckl files found in split folder: {split_folder}")
        return split_folder
    ...
    def train_dataloader(self):
        split_folder = self._require_split_folder('train')
        dt = EventDataset(split_folder, chunk_len_ms = self.chunk_len_ms, 
                           validation=False, 
    ...
    def val_dataloader(self):
        split_folder = self._require_split_folder('val')
        dt = EventDataset(split_folder, chunk_len_ms = self.chunk_len_ms, 
                           validation=True, 
                           preproc_polarity=self.preproc_polarity, patch_size=self.patch_size,
                           min_activations_per_patch=self.min_activations_per_patch,
